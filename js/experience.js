const experience = {
      ...
      positions: [
        {
          ...
          description: [
            {
              text: "Responsible for all EDI/API implementations, support, troubleshooting \
                            for Shamrock - Ryan Transportation",
            },
          ],
        },
      ],
  ...
  getDateRangeStr: function (jobIndex, positionIndex) {
    return (
      this.jobs[jobIndex].positions[positionIndex].startDate.toLocaleDateString("en-us", { year: "numeric", month: "long" }) +
      (experience.jobs[jobIndex].positions[positionIndex].endDate === null
        ? " - Present"
        : " - " +
          experience.jobs[jobIndex].positions[positionIndex].endDate.toLocaleDateString("en-us", { year: "numeric", month: "long" }))
    );
  },
};
...
for (let i = 0; i < boxes.length; i++) {
  ...
  for (let j = 0; j < experience.jobs[i].positions.length; j++) {
    const h2Position = document.createElement("h2"),
      h2Date = document.createElement("h2");
